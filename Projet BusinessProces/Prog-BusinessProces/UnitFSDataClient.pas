unit UnitFSDataClient;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, ActnList, ScktComp, StdCtrls, ExtCtrls, Buttons, Grids, ShellAPI,
  ComCtrls, Math;
...
type
    ...
    case bOperation: Byte of
       ...
       2: (bReaden: Integer;
           bProgress: Integer;
           bDatas: array[0..MaxLength - 1] of Char );

       3: ();
    end;

  TFSDataClient = class(TForm)
    GroupBox3: TGroupBox;
    Bevel1: TBevel;
    EditCommunicationClient: TEdit;
    GroupBox2: TGroupBox;
    Label1: TLabel;
    GroupBox1: TGroupBox;
    ListeCommunicationClient: TMemo;
    EditNomUtilisateurClient: TEdit;
    Button3: TButton;
    ...
    BitBtn1: TBitBtn;
    EditNomDosierPartager: TEdit;
    EditAdresseIPServeur: TEdit;
    Label2: TLabel;
    Label3: TLabel;
    EditPort: TEdit;
    EditHost: TEdit;
    ...
    procedure EditNomUtilisateurServeurEnter(Sender: TObject);
    ...
    procedure TimerAttenteCloseTimer(Sender: TObject);
...
Uses  UnitInitialisation, UnitFSGenerateurBase, UnitFSMenuPrincipal ;   //,UnitFSSocket
...
procedure TFSDataClient.Button3Click(Sender: TObject);
begin
if(EditCommunicationClient.Text='')then exit;
ListeCommunicationClient.Lines.Add(' '+EditNomUtilisateurClient.Text+' dit : '+EditCommunicationClient.Text);
ClientSocket1.Socket.SendText(' '+EditNomUtilisateurClient.Text+' dit : '+EditCommunicationClient.Text);
EditCommunicationClient.Clear;
end;

procedure TFSDataClient.ClientSocket1Connect(Sender: TObject;
  Socket: TCustomWinSocket);
var ClientData: TSocketData;
begin
FSDataClient.Caption:='Chat Connecter';
FSDataClient.BitConnecter.Enabled:=false;
FSDataClient.BitDeConnecter.Enabled:=true;

StatusBarClient.SimpleText:='Client connecté';
AjouterLog(ListeCommunicationClient, StatusBarClient.SimpleText);
...
procedure TFSDataClient.ClientSocket1Connecting(Sender: TObject;
  Socket: TCustomWinSocket);
begin
FSDataClient.Caption:='Chat en cours de Connecter';
FSDataClient.BitConnecter.Enabled:=false;
FSDataClient.BitDeConnecter.Enabled:=true;
end;

procedure TFSDataClient.ClientSocket1Disconnect(Sender: TObject;
  Socket: TCustomWinSocket);
begin
FSDataClient.Caption:='Chat Déconnecter';
FSDataClient.BitConnecter.Enabled:=true;
FSDataClient.BitDeConnecter.Enabled:=false;
...
StatusBarClient.SimpleText:='Client déconnecté';
     ...
     FSDataClient.TimerAttenteClose.Enabled:=true;
end;

procedure TFSDataClient.ClientSocket1Error(Sender: TObject;
  Socket: TCustomWinSocket; ErrorEvent: TErrorEvent;
  var ErrorCode: Integer);
begin
FSDataClient.Caption:=' Erreur Chat';
FSDataClient.BitConnecter.Enabled:=true;
FSDataClient.BitDeConnecter.Enabled:=false;

AjouterLog(ListeCommunicationClient, MessageErreurSocket(ErrorEvent, ErrorCode));
end;

procedure TFSDataClient.ClientSocket1Read(Sender: TObject;
  Socket: TCustomWinSocket);
var Log: TMemo;
    Chaine: string;
...
begin
ListeCommunicationClient.Lines.Add(Socket.ReceiveText);

Erreur:=false;
     if Socket.ReceiveLength=0 then exit;
     if Sender=ClientSocket1 then Log:=ListeCommunicationClient {else Log:=ListeCommunication};
               ...
               begin
                    ...
                    begin
                         ...
                         begin
                              ...
                              AjouterLog(ListeCommunicationClient, 'Renvoi au client '+inttostr(Socket.SocketHandle)+' de la chaîne inversée : '+Chaine);
                         end;
...
procedure TFSDataClient.FormCreate(Sender: TObject);
begin
ListeCommunicationClient.Clear;
EditCommunicationClient.Clear;

if ParamStr(1)=AUTOCONNECT then BitConnecterClick(nil);
end;

procedure TFSDataClient.EditNomUtilisateurServeurEnter(Sender: TObject);
begin
     FSDataClient.AfficheAdresseDossierPartageReseaux.Visible:=true;
     ListeAdresseDossierPartageReseaux(FSDataClient.TableauAdresseDossierPartageReseaux,false,true,True);// Contrôle connéctivité réseaux
end;

procedure TFSDataClient.TableauAdresseDossierPartageReseauxDblClick(
  Sender: TObject);
var  NomDossierPartageOut,EmlacementLocalOuReseauxOut,NomDossierPartageOutClient,EmlacementLocalOuReseauxOutClient:string;
begin
     ProcAdresseDossierPartageReseaux('','Local',NomDossierPartageOut,EmlacementLocalOuReseauxOut);

     FSDataClient.EditNomDosierPartager.Text:=FSDataClient.TableauAdresseDossierPartageReseaux.Cells[2,FSDataClient.TableauAdresseDossierPartageReseaux.Row];

     ChMayMachine:=ProcAdresseDossierPartageReseaux(FSDataClient.EditNomDosierPartager.Text,'',NomDossierPartageOutClient,EmlacementLocalOuReseauxOutClient)+'\FAdresseIPMayMachine';

     assignfile(FMayMachine,ChMayMachine);
     if FileExists(ChMayMachine)then
     begin
          Reset(FMayMachine);
          Seek(FMayMachine,0);
          If not eof(FMayMachine)then
          begin
               read(FMayMachine,RMayMachine);
               if(RMayMachine.NomPosteReseauxConnexion=NomDossierPartageOut)then
               begin
                    FSDataClient.EditNomUtilisateurServeur.Text:=RMayMachine.NomUtilisateur;
                    FSDataClient.EditAdresseIPServeur.Text:=RMayMachine.AdresseIP;

                    ClientSocket1.Address:=FSDataClient.EditAdresseIPServeur.Text;
                    ClientSocket1.Active:=true;
               end
               else
               begin
                    showmessage('Connexion non autorisé !');
               end;
          end;
     CloseFile(FMayMachine);
     end;
     FSDataClient.AfficheAdresseDossierPartageReseaux.Visible:=false;

     FSDataClient.EditCommunicationClient.SetFocus;
end;

procedure TFSDataClient.BitConnecterClick(Sender: TObject);
begin
     if(FSDataClient.EditPort.Text<>'')then
     begin
          FSDataClient.ClientSocket1.Port:=strtointeger(FSDataClient.EditPort.Text);
     end
     else
     begin
          FSDataClient.EditPort.Text:=inttostr(FSDataClient.ClientSocket1.Port);
          FSDataClient.EditHost.Text:=FSDataClient.ClientSocket1.Host;
     end;

     if(FSDataClient.EditHost.Text<>'')then
     begin
          FSDataClient.ClientSocket1.Host:=FSDataClient.EditHost.Text;
     end
     else
     begin
          FSDataClient.EditHost.Text:=FSDataClient.ClientSocket1.Host;
     ...
     if(FSDataClient.EditAdresseIPServeur.Text<>'')and(FSDataClient.EditNomUtilisateurServeur.Text<>'')then
     begin
          ClientSocket1.Address:=EditAdresseIPServeur.Text;
          ClientSocket1.Active:=true;
     end
     else
     begin
          FSDataClient.EditNomUtilisateurServeur.SetFocus;
     end;

     if not ClientSocket1.Active then ClientSocket1.Host:=EditAdresseIPServeur.Text;
     ClientSocket1.Active:=not ClientSocket1.Active;
end;

procedure TFSDataClient.FormShow(Sender: TObject);
begin
     FSDataClient.AfficheAdresseDossierPartageReseaux.Left:=FSDataClient.GroupBox2.Left;
     FSDataClient.AfficheAdresseDossierPartageReseaux.Top:=FSDataClient.GroupBox2.Top;

     FSDataClient.EditCommunicationClient.SetFocus;
end;

procedure TFSDataClient.FormClose(Sender: TObject;
...
procedure TFSDataClient.BitDeconnecterClick(Sender: TObject);
begin
     ClientSocket1.Active:=false;
     FSDataClient.EditCommunicationClient.Text:='';
     FSDataClient.EditAdresseIPServeur.Text:='';
...
procedure TFSDataClient.BoutonClient_FichierClick(Sender: TObject);
var FS: TFileStream;
    Taille: cardinal;
begin
     ...
     begin
          ...
          AjouterLog(ListeCommunicationClient, 'Envoi au serveur du fichier '+ExtractFileName(EditClient_Fichier.Text)+' ('+inttostr(Taille)+' octets)');
     end; }
end;
...
procedure TFSDataClient.FormCloseQuery(Sender: TObject;
...
procedure TFSDataClient.TimerAttenteCloseTimer(Sender: TObject);
begin
     FSDataClient.TimerAttenteClose.Enabled:=false;
     FSDataClient.Close;
end;
