(() => {
  ...
  // src/app/pages/admin/AdminDocsEditor.tsx
  var import_react2 = __require("react");
  var import_react_router2 = __require("react-router");
...
${suffix}`;
  ...
  function Card({
    ...
    return /* @__PURE__ */ React.createElement(
      import_framer_motion.motion.div,
      {
        ...
        className: `group relative bg-card border border-border/50 ${overflowVisible ? "" : "overflow-hidden"} ${hover ? "transition-all duration-300 hover:-translate-y-1 hover:border-secondary/30" : ""} ${className}`
      },
      /* @__PURE__ */ React.createElement(
        import_framer_motion.motion.div,
        {
          ...
          style: {
            background: import_framer_motion.useMotionTemplate`
            radial-gradient(
      ...
      /* @__PURE__ */ React.createElement("div", { className: "relative z-10 h-full" }, children)
    );
  }
  ...
  function Button({
    ...
    const baseStyles = "relative inline-flex items-center justify-center font-body font-medium tracking-wide transition-all disabled:opacity-50 disabled:cursor-not-allowed select-none";
    const variants = {
      ...
      outline: "border border-foreground/20 bg-transparent hover:border-secondary hover:text-secondary text-foreground",
    ...
    if (href) {
      return /* @__PURE__ */ React.createElement(
        ...
        /* @__PURE__ */ React.createElement(import_react_router.Link, { to: href, target, rel, className: classes }, /* @__PURE__ */ React.createElement("span", { className: "flex items-center gap-2" }, children))
      );
    }
    return /* @__PURE__ */ React.createElement(
      ...
      /* @__PURE__ */ React.createElement("span", { className: "flex items-center gap-2" }, children)
    );
  }

  // src/app/pages/admin/AdminDocsEditor.tsx
  var import_lucide_react = __require("lucide-react");
  var import_sonner = __require("sonner");
  var import_framer_motion3 = __require("framer-motion");
  var import_dnd = __require("@hello-pangea/dnd");
  function AdminDocsEditor() {
    const { id: pageId } = (0, import_react_router2.useParams)();
    ...
    const [reorderMode, setReorderMode] = (0, import_react2.useState)("buttons");
    const [uploading, setUploading] = (0, import_react2.useState)(null);
    const fetchContent = (0, import_react2.useCallback)(async () => {
      ...
      if (sectionsRes.data) {
        const formatted = sectionsRes.data.map((s) => ({
          ...s,
          subsections: (s.docs_subsections || []).sort((a, b) => a.order_index - b.order_index),
          isCollapsed: false
        }));
    ...
    const addSection = async () => {
      ...
      if (error) import_sonner.toast.error("Erreur cr\xE9ation section");
    ...
    const deleteSection = async (id) => {
      ...
      if (!error) {
        ...
        import_sonner.toast.success("Section supprim\xE9e");
      }
    };
    const addBlock = async (sectionId) => {
      ...
      if (!error) {
        ...
        import_sonner.toast.success("Bloc ajout\xE9");
      }
    };
    const deleteBlock = async (id, sectionId) => {
      ...
      if (!error) {
        ...
        import_sonner.toast.success("Bloc supprim\xE9");
      }
    };
    const updateSectionTitle = (id, title) => {
      setSections(sections.map((s) => s.id === id ? { ...s, title, hasChanges: true } : s));
    };
    const updateBlock = (sectionId, blockId, data) => {
    ...
    const toggleBlock = (sectionId, blockId) => {
      setSections(sections.map((s) => s.id === sectionId ? {
        ...s,
        subsections: s.subsections.map((b) => b.id === blockId ? { ...b, isCollapsed: !b.isCollapsed } : b)
      } : s));
    };
    const setBlockType = (sectionId, blockId, type) => {
    ...
    const saveSection = async (section) => {
      ...
      if (sError || bError) {
        import_sonner.toast.error("Erreur lors de l'enregistrement");
      } else {
        setSections(sections.map((s) => s.id === id ? { ...s, hasChanges: false } : s));
        import_sonner.toast.success("Section synchronis\xE9e !");
      }
      setSaving(false);
    };
    const handleFileUpload = async (sectionId, blockId, file) => {
      const fileExt = file.name.split(".").pop();
      const fileName = `${Math.random()}.${fileExt}`;
      const filePath = `docs/${fileName}`;
      setUploading(blockId);
      const { error: uploadError } = await supabase.storage.from("docs_media").upload(filePath, file);
      if (uploadError) {
        import_sonner.toast.error("Erreur d'upload");
        setUploading(null);
        return;
      }
      const { data: { publicUrl } } = supabase.storage.from("docs_media").getPublicUrl(filePath);
      updateBlock(sectionId, blockId, { image_url: publicUrl });
      setUploading(null);
      import_sonner.toast.success("M\xE9dia t\xE9l\xE9vers\xE9 !");
    };
    const handleStepImageUpload = async (sectionId, blockId, stepIndex, file) => {
      const fileExt = file.name.split(".").pop();
      const fileName = `${Math.random()}.${fileExt}`;
      const filePath = `docs/${fileName}`;
      ...
      setSections((prev) => {
        ...
        let steps = [];
        try {
          steps = JSON.parse(block.content || "[]");
        } catch (e) {
        }
    ...
    const onDragEnd = async (result) => {
      ...
      if (type === "section") {
        ...
        await Promise.all(updated.map(
          (s) => supabase.from("docs_sections").update({ order_index: s.order_index }).eq("id", s.id)
        ));
      } else {
    ...
    const moveItem = (type, id, direction, sectionId) => {
      if (type === "section") {
    ...
    if (loading) return /* @__PURE__ */ React.createElement("div", { className: "flex flex-col items-center justify-center h-[60vh] gap-4" }, /* @__PURE__ */ React.createElement(import_lucide_react.Loader2, { className: "w-10 h-10 animate-spin text-primary" }), /* @__PURE__ */ React.createElement("p", { className: "text-muted-foreground animate-pulse" }, "Chargement de l'\xE9diteur..."));
    return /* @__PURE__ */ React.createElement("div", { className: "container mx-auto px-4 pb-32 animate-in fade-in slide-in-from-bottom-4 duration-700" }, /* @__PURE__ */ React.createElement("header", { className: "flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12 sticky top-4 z-50 bg-background/80 backdrop-blur-xl p-6 rounded-[2rem] border border-border/50 shadow-2xl" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-4" }, /* @__PURE__ */ React.createElement("button", { onClick: () => navigate("/celestial-cms/docs"), className: "p-3 rounded-2xl bg-muted/50 hover:bg-muted transition-all" }, /* @__PURE__ */ React.createElement(import_lucide_react.ArrowLeft, { className: "w-5 h-5" })), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h1", { className: "text-2xl font-black tracking-tight" }, pageInfo?.title || "\xC9diteur"), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-muted-foreground uppercase tracking-widest font-bold opacity-50" }, sections.length, " sections au total"))), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-3 w-full md:w-auto" }, /* @__PURE__ */ React.createElement("div", { className: "flex bg-muted p-1 rounded-2xl border border-border/50" }, /* @__PURE__ */ React.createElement(
      "button",
      {
      ...
      /* @__PURE__ */ React.createElement(import_lucide_react.Settings2, { className: "w-3.5 h-3.5" }),
      " Boutons"
    ), /* @__PURE__ */ React.createElement(
      "button",
      {
      ...
      /* @__PURE__ */ React.createElement(import_lucide_react.GripVertical, { className: "w-3.5 h-3.5" }),
      " Drag & Drop"
    )), /* @__PURE__ */ React.createElement(Button, { onClick: addSection, className: "rounded-2xl shrink-0" }, /* @__PURE__ */ React.createElement(import_lucide_react.Plus, { className: "w-4 h-4 mr-2" }), " Section"))), /* @__PURE__ */ React.createElement(import_dnd.DragDropContext, { onDragEnd }, /* @__PURE__ */ React.createElement(import_dnd.Droppable, { droppableId: "all-sections", type: "section" }, (provided) => /* @__PURE__ */ React.createElement("div", { ...provided.droppableProps, ref: provided.innerRef, className: "space-y-12" }, sections.map((section, sIdx) => /* @__PURE__ */ React.createElement(import_dnd.Draggable, { key: section.id, draggableId: section.id, index: sIdx, isDragDisabled: reorderMode === "buttons" }, (provided2) => /* @__PURE__ */ React.createElement("div", { ref: provided2.innerRef, ...provided2.draggableProps, className: "group/section relative" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-4 mb-6 group/h" }, reorderMode === "dnd" && /* @__PURE__ */ React.createElement("div", { ...provided2.dragHandleProps, className: "p-2 text-muted-foreground/30 hover:text-primary transition-colors cursor-grab active:cursor-grabbing" }, /* @__PURE__ */ React.createElement(import_lucide_react.GripVertical, { className: "w-5 h-5" })), reorderMode === "buttons" && /* @__PURE__ */ React.createElement("div", { className: "flex flex-col gap-1 items-center bg-muted/30 p-1 rounded-lg" }, /* @__PURE__ */ React.createElement("button", { onClick: () => moveItem("section", section.id, "up"), className: "p-1 hover:bg-primary/10 hover:text-primary rounded text-muted-foreground disabled:opacity-30", disabled: sIdx === 0 }, /* @__PURE__ */ React.createElement(import_lucide_react.ChevronUp, { className: "w-4 h-4" })), /* @__PURE__ */ React.createElement("button", { onClick: () => moveItem("section", section.id, "down"), className: "p-1 hover:bg-primary/10 hover:text-primary rounded text-muted-foreground disabled:opacity-30", disabled: sIdx === sections.length - 1 }, /* @__PURE__ */ React.createElement(import_lucide_react.ChevronDown, { className: "w-4 h-4" }))), /* @__PURE__ */ React.createElement(
      "button",
      {
      ...
      section.isCollapsed ? /* @__PURE__ */ React.createElement(import_lucide_react.ChevronRight, { className: "w-4 h-4" }) : /* @__PURE__ */ React.createElement(import_lucide_react.ChevronDown, { className: "w-4 h-4" }),
      /* @__PURE__ */ React.createElement("span", { className: "text-[10px] font-black uppercase tracking-widest opacity-60" }, section.subsections.length, " blocs")
    ), /* @__PURE__ */ React.createElement(
      "input",
      {
        value: section.title,
    ...
    ), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2" }, /* @__PURE__ */ React.createElement(import_framer_motion3.AnimatePresence, null, section.hasChanges && /* @__PURE__ */ React.createElement(import_framer_motion3.motion.div, { initial: { opacity: 0, scale: 0.8 }, animate: { opacity: 1, scale: 1 }, exit: { opacity: 0, scale: 0.8 } }, /* @__PURE__ */ React.createElement(Button, { size: "sm", onClick: () => saveSection(section), className: "rounded-xl shadow-lg shadow-primary/20", disabled: saving }, saving ? /* @__PURE__ */ React.createElement(import_lucide_react.Loader2, { className: "w-4 h-4 animate-spin mr-2" }) : /* @__PURE__ */ React.createElement(import_lucide_react.Save, { className: "w-4 h-4 mr-2" }), "Sauver Section"))), /* @__PURE__ */ React.createElement("button", { onClick: () => deleteSection(section.id), className: "p-2.5 text-muted-foreground hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all opacity-0 group-hover/h:opacity-100" }, /* @__PURE__ */ React.createElement(import_lucide_react.Trash2, { className: "w-5 h-5" })))), /* @__PURE__ */ React.createElement(import_framer_motion3.AnimatePresence, { initial: false }, !section.isCollapsed && /* @__PURE__ */ React.createElement(
      ...
      /* @__PURE__ */ React.createElement(import_dnd.Droppable, { droppableId: section.id, type: "block" }, (provided3) => /* @__PURE__ */ React.createElement("div", { ...provided3.droppableProps, ref: provided3.innerRef, className: "space-y-8 ml-8 border-l-2 border-border/30 pl-10 group-hover/section:border-primary/20 transition-all" }, section.subsections.map((block, bIdx) => /* @__PURE__ */ React.createElement(import_dnd.Draggable, { key: block.id, draggableId: block.id, index: bIdx, isDragDisabled: reorderMode === "buttons" }, (provided4) => /* @__PURE__ */ React.createElement("div", { ref: provided4.innerRef, ...provided4.draggableProps, className: "group/block relative" }, /* @__PURE__ */ React.createElement(Card, { className: "rounded-[2.5rem] border-border/60 hover:border-primary/20 hover:shadow-2xl transition-all overflow-hidden bg-card/50 backdrop-blur-sm" }, /* @__PURE__ */ React.createElement("div", { className: "flex min-h-[140px]" }, /* @__PURE__ */ React.createElement("div", { className: "w-14 flex-shrink-0 bg-muted/40 border-r border-border/50 flex flex-col items-center py-6 gap-6" }, reorderMode === "dnd" ? /* @__PURE__ */ React.createElement("div", { ...provided4.dragHandleProps, className: "p-2 text-muted-foreground/30 hover:text-primary transition-colors cursor-grab active:cursor-grabbing" }, /* @__PURE__ */ React.createElement(import_lucide_react.GripVertical, { className: "w-5 h-5" })) : /* @__PURE__ */ React.createElement("div", { className: "flex flex-col gap-2 bg-background/50 p-1 rounded-xl border border-border/50" }, /* @__PURE__ */ React.createElement("button", { onClick: () => moveItem("block", block.id, "up", section.id), className: "p-1 hover:bg-primary/10 hover:text-primary rounded-lg text-muted-foreground disabled:opacity-20", disabled: bIdx === 0 }, /* @__PURE__ */ React.createElement(import_lucide_react.ChevronUp, { className: "w-4 h-4" })), /* @__PURE__ */ React.createElement("button", { onClick: () => moveItem("block", block.id, "down", section.id), className: "p-1 hover:bg-primary/10 hover:text-primary rounded-lg text-muted-foreground disabled:opacity-20", disabled: bIdx === section.subsections.length - 1 }, /* @__PURE__ */ React.createElement(import_lucide_react.ChevronDown, { className: "w-4 h-4" }))), /* @__PURE__ */ React.createElement("button", { onClick: () => deleteBlock(block.id, section.id), className: "p-3 text-muted-foreground/40 hover:text-red-500 hover:bg-red-500/10 rounded-2xl transition-all" }, /* @__PURE__ */ React.createElement(import_lucide_react.Trash2, { className: "w-5 h-5" }))), /* @__PURE__ */ React.createElement("div", { className: "flex-1 p-8 space-y-8" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-4" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2 p-1.5 bg-muted/50 rounded-2xl w-fit border border-border/50" }, [
        { id: "title", icon: /* @__PURE__ */ React.createElement(import_lucide_react.Heading1, { className: "w-4 h-4" }), label: "Titre" },
        { id: "subtitle", icon: /* @__PURE__ */ React.createElement(import_lucide_react.Heading2, { className: "w-4 h-4" }), label: "Sous-titre" },
        { id: "text", icon: /* @__PURE__ */ React.createElement(import_lucide_react.AlignLeft, { className: "w-4 h-4" }), label: "Texte" },
        { id: "list", icon: /* @__PURE__ */ React.createElement(import_lucide_react.List, { className: "w-4 h-4" }), label: "Liste" },
        { id: "step_by_step", icon: /* @__PURE__ */ React.createElement(import_lucide_react.ListOrdered, { className: "w-4 h-4" }), label: "\xC9tapes" },
        { id: "hr", icon: /* @__PURE__ */ React.createElement(import_lucide_react.Minus, { className: "w-4 h-4" }), label: "Ligne" },
        { id: "photo", icon: /* @__PURE__ */ React.createElement(import_lucide_react.Image, { className: "w-4 h-4" }), label: "Photo" },
        { id: "video", icon: /* @__PURE__ */ React.createElement(import_lucide_react.Video, { className: "w-4 h-4" }), label: "Vid\xE9o" },
        { id: "code", icon: /* @__PURE__ */ React.createElement(import_lucide_react.Code2, { className: "w-4 h-4" }), label: "Code" }
      ].map((t) => /* @__PURE__ */ React.createElement(
        "button",
        {
          key: t.id,
          onClick: () => setBlockType(section.id, block.id, t.id),
        ...
        t.icon,
        " ",
        /* @__PURE__ */ React.createElement("span", { className: "hidden sm:inline" }, t.label)
      ))), /* @__PURE__ */ React.createElement(
        "button",
        {
        ...
        block.isCollapsed ? /* @__PURE__ */ React.createElement(import_lucide_react.ChevronRight, { className: "w-4 h-4" }) : /* @__PURE__ */ React.createElement(import_lucide_react.ChevronDown, { className: "w-4 h-4" }),
        /* @__PURE__ */ React.createElement("span", { className: "text-[10px] font-black uppercase tracking-widest opacity-60" }, block.isCollapsed ? "D\xE9plier" : "R\xE9duire")
      )), !block.isCollapsed && /* @__PURE__ */ React.createElement("div", { className: "space-y-6 animate-in fade-in slide-in-from-top-4 duration-300" }, (block.type === "title" || block.type === "subtitle" || !block.type || block.type === "text") && /* @__PURE__ */ React.createElement(
        "input",
        {
          value: block.title,
      ...
      ), (block.type === "subtitle" || block.type === "text" || !block.type) && /* @__PURE__ */ React.createElement(
        "textarea",
        {
          value: block.content,
          ...
          placeholder: "\xC9crivez ici le contenu de votre bloc...",
          rows: block.type === "subtitle" ? 2 : 4
        }
      ), block.type === "list" && /* @__PURE__ */ React.createElement("div", { className: "space-y-3" }, /* @__PURE__ */ React.createElement(
        "input",
        {
          value: block.title,
          onChange: (e) => updateBlock(section.id, block.id, { title: e.target.value }),
      ...
      ), /* @__PURE__ */ React.createElement("div", { className: "rounded-xl bg-muted/30 border border-border/40 p-4" }, /* @__PURE__ */ React.createElement("p", { className: "text-[9px] font-black uppercase tracking-widest text-muted-foreground/50 mb-2" }, "Une entree par ligne (ex: - Element)"), /* @__PURE__ */ React.createElement(
        "textarea",
        {
          value: block.content,
          onChange: (e) => updateBlock(section.id, block.id, { content: e.target.value }),
      ...
      )), block.content && /* @__PURE__ */ React.createElement("div", { className: "rounded-xl bg-card border border-border/30 p-4" }, /* @__PURE__ */ React.createElement("p", { className: "text-[9px] font-black uppercase tracking-widest text-primary/60 mb-3" }, "Apercu"), /* @__PURE__ */ React.createElement("ul", { className: "space-y-1.5" }, block.content.split("\n").filter((l) => l.trim()).map((line, i) => /* @__PURE__ */ React.createElement("li", { key: i, className: "flex items-start gap-2 text-sm text-muted-foreground" }, /* @__PURE__ */ React.createElement("span", { className: "w-1.5 h-1.5 rounded-full bg-primary shrink-0 mt-1.5" }), line.replace(/^[-*\u2022]\s*/, "")))))), block.type === "step_by_step" && (() => {
        let steps = [];
        try {
          steps = JSON.parse(block.content || "[]");
        } catch (e) {
        ...
        return /* @__PURE__ */ React.createElement("div", { className: "space-y-4" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between" }, /* @__PURE__ */ React.createElement(
          "input",
          {
            value: block.title,
            onChange: (e) => updateBlock(section.id, block.id, { title: e.target.value }),
            className: "text-lg font-bold bg-transparent border-none outline-none focus:text-primary",
            placeholder: "Titre global des \xE9tapes (optionnel)..."
          }
        ), /* @__PURE__ */ React.createElement(Button, { size: "sm", variant: "outline", onClick: addStep, className: "gap-2" }, /* @__PURE__ */ React.createElement(import_lucide_react.Plus, { className: "w-4 h-4" }), " Ajouter une \xE9tape")), /* @__PURE__ */ React.createElement("div", { className: "space-y-4" }, steps.map((step, idx) => /* @__PURE__ */ React.createElement("div", { key: idx, className: "p-4 rounded-xl border border-border/50 bg-muted/20 relative group/step" }, /* @__PURE__ */ React.createElement("div", { className: "absolute top-2 right-2 opacity-0 group-hover/step:opacity-100 transition-opacity" }, /* @__PURE__ */ React.createElement("button", { onClick: () => removeStep(idx), className: "p-2 text-red-500 hover:bg-red-500/10 rounded-lg" }, /* @__PURE__ */ React.createElement(import_lucide_react.Trash2, { className: "w-4 h-4" }))), /* @__PURE__ */ React.createElement("div", { className: "flex gap-4 items-start" }, /* @__PURE__ */ React.createElement("div", { className: "w-8 h-8 shrink-0 rounded-full bg-primary/20 text-primary font-black flex items-center justify-center" }, idx + 1), /* @__PURE__ */ React.createElement("div", { className: "flex-1 space-y-3" }, /* @__PURE__ */ React.createElement(
          "input",
          {
            value: step.title || "",
            onChange: (e) => updateStep(idx, { title: e.target.value }),
            className: "text-base font-bold bg-transparent border-none outline-none focus:text-primary w-full",
            placeholder: `Titre de l'\xE9tape ${idx + 1}...`
          }
        ), /* @__PURE__ */ React.createElement(
          "textarea",
          {
            ...
            placeholder: "Description de l'\xE9tape...",
            rows: 2
          }
        )), /* @__PURE__ */ React.createElement("div", { className: "w-32 h-24 shrink-0 rounded-lg bg-muted border border-dashed border-border flex flex-col items-center justify-center gap-2 group/drop relative overflow-hidden" }, step.img ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("img", { src: step.img, alt: "Preview", className: "absolute inset-0 w-full h-full object-cover" }), /* @__PURE__ */ React.createElement("div", { className: "absolute inset-0 bg-black/40 opacity-0 group-hover/drop:opacity-100 transition-opacity flex items-center justify-center" }, /* @__PURE__ */ React.createElement(import_lucide_react.Upload, { className: "w-5 h-5 text-white" }))) : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(import_lucide_react.Image, { className: "w-5 h-5 text-muted-foreground/30" }), /* @__PURE__ */ React.createElement("span", { className: "text-[9px] uppercase tracking-wider font-bold text-muted-foreground/50" }, "Photo")), /* @__PURE__ */ React.createElement("input", { type: "file", className: "absolute inset-0 opacity-0 cursor-pointer", onChange: (e) => e.target.files?.[0] && handleStepImageUpload(section.id, block.id, idx, e.target.files[0]) }), uploading === `${block.id}-${idx}` && /* @__PURE__ */ React.createElement("div", { className: "absolute inset-0 bg-background/80 flex items-center justify-center" }, /* @__PURE__ */ React.createElement(import_lucide_react.Loader2, { className: "w-5 h-5 animate-spin" })))))), steps.length === 0 && /* @__PURE__ */ React.createElement("div", { className: "text-center p-8 border-2 border-dashed border-border rounded-xl text-muted-foreground" }, "Aucune \xE9tape pour le moment.")));
      })(), block.type === "photo" && /* @__PURE__ */ React.createElement("div", { className: "grid md:grid-cols-[200px_1fr] gap-8" }, /* @__PURE__ */ React.createElement("div", { className: "aspect-square rounded-3xl bg-muted border-2 border-dashed border-border flex flex-col items-center justify-center gap-4 group/drop relative overflow-hidden" }, block.image_url ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("img", { src: block.image_url, alt: "Preview", className: "absolute inset-0 w-full h-full object-cover transition-transform group-hover/drop:scale-110" }), /* @__PURE__ */ React.createElement("div", { className: "absolute inset-0 bg-black/40 opacity-0 group-hover/drop:opacity-100 transition-opacity flex items-center justify-center" }, /* @__PURE__ */ React.createElement(import_lucide_react.Upload, { className: "w-8 h-8 text-white" }))) : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(import_lucide_react.Image, { className: "w-8 h-8 text-muted-foreground/30" }), /* @__PURE__ */ React.createElement("p", { className: "text-[10px] font-black uppercase tracking-widest text-muted-foreground/50" }, "Logo / Photo")), /* @__PURE__ */ React.createElement("input", { type: "file", className: "absolute inset-0 opacity-0 cursor-pointer", onChange: (e) => e.target.files?.[0] && handleFileUpload(section.id, block.id, e.target.files[0]) }), uploading === block.id && /* @__PURE__ */ React.createElement("div", { className: "absolute inset-0 bg-background/80 flex items-center justify-center" }, /* @__PURE__ */ React.createElement(import_lucide_react.Loader2, { className: "w-6 h-6 animate-spin" }))), /* @__PURE__ */ React.createElement("div", { className: "space-y-4" }, /* @__PURE__ */ React.createElement("input", { value: block.image_url, onChange: (e) => updateBlock(section.id, block.id, { image_url: e.target.value }), className: "w-full bg-muted/50 px-4 py-2 rounded-xl text-sm border-none outline-none", placeholder: "URL de l'image" }), /* @__PURE__ */ React.createElement("input", { value: block.image_alt, onChange: (e) => updateBlock(section.id, block.id, { image_alt: e.target.value }), className: "w-full bg-muted/50 px-4 py-2 rounded-xl text-sm border-none outline-none", placeholder: "Texte alternatif (SEO)" }), /* @__PURE__ */ React.createElement("input", { value: block.image_caption, onChange: (e) => updateBlock(section.id, block.id, { image_caption: e.target.value }), className: "w-full bg-muted/50 px-4 py-2 rounded-xl text-sm border-none outline-none", placeholder: "L\xE9gende discr\xE8te" }))), block.type === "video" && /* @__PURE__ */ React.createElement("div", { className: "space-y-4" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-4" }, /* @__PURE__ */ React.createElement("div", { className: "p-3 rounded-2xl bg-red-500/10 text-red-500" }, /* @__PURE__ */ React.createElement(import_lucide_react.Play, { className: "w-6 h-6" })), /* @__PURE__ */ React.createElement("input", { value: block.video_url, onChange: (e) => updateBlock(section.id, block.id, { video_url: e.target.value }), className: "flex-1 bg-muted/50 px-6 py-3 rounded-2xl text-sm font-bold border-none outline-none focus:bg-muted", placeholder: "Lien vers la vid\xE9o (MP4 ou YouTube Embed)" })), /* @__PURE__ */ React.createElement("input", { value: block.video_title, onChange: (e) => updateBlock(section.id, block.id, { video_title: e.target.value }), className: "w-full bg-muted/30 px-6 py-2 rounded-xl text-xs border-none outline-none", placeholder: "Titre de la vid\xE9o" })), block.type === "code" && /* @__PURE__ */ React.createElement("div", { className: "rounded-2xl overflow-hidden border border-border/50 bg-[#0d0f14] shadow-2xl" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between px-6 py-3 border-b border-white/5 bg-white/5" }, /* @__PURE__ */ React.createElement("select", { value: block.code_language, onChange: (e) => updateBlock(section.id, block.id, { code_language: e.target.value }), className: "bg-transparent text-[10px] font-black uppercase tracking-widest text-white/50 outline-none border-none" }, /* @__PURE__ */ React.createElement("option", { value: "javascript" }, "JavaScript"), /* @__PURE__ */ React.createElement("option", { value: "typescript" }, "TypeScript"), /* @__PURE__ */ React.createElement("option", { value: "python" }, "Python"), /* @__PURE__ */ React.createElement("option", { value: "bash" }, "Terminal / Bash"), /* @__PURE__ */ React.createElement("option", { value: "sql" }, "SQL"), /* @__PURE__ */ React.createElement("option", { value: "html" }, "HTML"), /* @__PURE__ */ React.createElement("option", { value: "css" }, "CSS")), /* @__PURE__ */ React.createElement(import_lucide_react.Code2, { className: "w-4 h-4 text-white/20" })), /* @__PURE__ */ React.createElement(
        "textarea",
        {
      ...
      )), block.type === "hr" && /* @__PURE__ */ React.createElement("div", { className: "py-8 flex items-center justify-center" }, /* @__PURE__ */ React.createElement("div", { className: "w-full max-w-sm h-[2px] bg-[#000000] relative" }, /* @__PURE__ */ React.createElement("div", { className: "absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-card px-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground/30" }, "S\xE9parateur")))))))))), provided3.placeholder, /* @__PURE__ */ React.createElement(
        "button",
        {
        ...
        /* @__PURE__ */ React.createElement(import_lucide_react.Plus, { className: "w-4 h-4 inline-block mr-2" }),
        " Ajouter un bloc \xE0 cette section"
      )))
    ))))), provided.placeholder))), /* @__PURE__ */ React.createElement("div", { className: "mt-20 flex justify-center pb-20" }, /* @__PURE__ */ React.createElement(
      "button",
      {
        onClick: addSection,
      ...
      /* @__PURE__ */ React.createElement(import_lucide_react.Plus, { className: "w-6 h-6 group-hover:rotate-90 transition-transform duration-500" }),
      " Nouvelles Section"
    )));
  }
})();
